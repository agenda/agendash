...
const mongodb = require('mongodb'); // We rely on the Agenda's "mongodb", thus our package.json lists "*" as required version
  ...
  const getJobs = function(job, state, options) {
    ...
    if (options.query && options.property) {
      if (options.isObjectId) {
        ...
        preMatch[options.property] = parseInt(options.query, 10);
      } else {
