...
module.exports = function(agenda, options) {
  ...
  const getJobs = function(job, state, options) {
    ...
    if (options.query && options.property) {
      if (options.isObjectId) {
        preMatch[options.property] = ObjectID.createFromHexString(options.query);
  ...
  const getOverview = async() => {
    ...
    const states = {total: 0, running: 0, scheduled: 0, queued: 0, completed: 0, failed: 0, repeating: 0};
  ...
  const api = async function(job, state, {query: q, property, isObjectId, skip, limit}) {
    if (!agenda) {
    ...
    const [overview, jobs] = await Promise.all([getOverview(), getJobs(job, state, {query: q, property, isObjectId, skip, limit})]);
    const apiResponse = {
      overview,
  ...
  const requeueJobs = async jobIds => {
    ...
    const collection = agenda._collection.collection || agenda._collection;
    const jobs = await collection.find({_id: {$in: jobIds.map(jobId => collection.s.pkFactory(jobId))}}).toArray();
    if (jobs.length === 0) {
