...
module.exports = agendash => {
  ...
  app.get('/api', async(request, response) => {
    try {
      ...
      const apiResponse = await api(job, state, {query: q, property, isObjectId, skip, limit});
  ...
  app.post('/api/jobs/create', async(request, response) => {
    try {
      await createJob(request.body.jobName, request.body.jobSchedule, request.body.jobRepeatEvery, request.body.jobData);
