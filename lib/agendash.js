...
module.exports = function(agenda, options) {
  ...
  const getJobs = (job, state, limit, skip, query = '', property = '', isObjectId = false, callback) => {
    const preMatch = {};
    if (job) {
      preMatch.name = job;
    }
    ...
    if (query && property) {
      if (isObjectId) {
        preMatch[property] = mongodb.ObjectID.createFromHexString(query);
      } else {
        preMatch[property] = { '$regex': query, '$options': 'i' };
      }
    }
  ...
  const api = function (job, state, limit, skip, q, property, isObjectId, callback) {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    async.parallel({
      overview: getOverview,
      jobs: getJobs.bind(this, job, state, limit, skip, q, property, isObjectId)
    },
    (err, apiResponse) => {
      if (err) {
