...
module.exports = function (agenda, options) {
  ...
  function requeueJobs (jobIds, callback) {
    ...
    try {
      agenda._collection
      .find({_id: {$in: jobIds.map((jobId) => (agenda._collection.collection || agenda._collection).s.pkFactory(jobId))}})
  ...
  function deleteJobs (jobIds, callback) {
    ...
    try {
      agenda.cancel({_id: {$in: jobIds.map((jobId) => (agenda._collection.collection || agenda._collection).s.pkFactory(jobId))}}, function (err, deleted) {
        if (err || !deleted) {
