...
module.exports = function(agenda, options) {
  ...
  const agendaStatus = async function(action, callback) {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    if (action && !["start", "stop"].includes(action)) throw new Error("Invalid Action")
    action && await agenda[action]()
    const started = !!agenda._processInterval
    callback(null, {
